    ...
    for i in range(l):
        for j in range(l):
            ...
            if (country == "CZ"):
                ...
                url = "http://mapy.geoportal.gov.pl/wss/service/img/guest/ORTO/MapServer/WMSServer?&REQUEST=GetMap&TRANSPARENT=FALSE&FORMAT=image/png&VERSION=1.3.0&LAYERS=Raster&STYLES=&BBOX=" + str(x1) + "," + str(y1) + "," + str(x2) + "," + str(y2) + "&CRS=EPSG:4326&EXCEPTIONS=xml&WIDTH=2048&HEIGHT=2048"
            ortos.append(url)
            connerr = True
            while (connerr):
                try:
